...
}),checklistsController=angularApplication.controller("ChecklistsController",["$scope",function($scope){var checklistsCtrl=this,appData=window.checklistsData,parseLocation=function(location){var pair,i,pairs=location.substring(1).split("&"),obj={};for(i in pairs)""!==pairs[i]&&(pair=pairs[i].split("="),obj[decodeURIComponent(pair[0])]=decodeURIComponent(pair[1]));return obj},getEndpointWithToken=function(endpoint){return endpoint+"?jwt="+jwtToken},jwtToken=parseLocation(window.location.search).jwt;checklistsCtrl.checklists=appData.checklists;for(var i in checklistsCtrl.checklists){checklistsCtrl.checklists[i].editName=checklistsCtrl.checklists[i].name;for(var j in checklistsCtrl.checklists[i].items)checklistsCtrl.checklists[i].items[j].editText=checklistsCtrl.checklists[i].items[j].text}checklistsCtrl.newChecklist={name:"",editMode:!1},$scope.checklistsSortableOptions={handle:".checklist-wapper i.cl-icon-notebook-list",stop:function(e){var ids=[];for(var i in checklistsCtrl.checklists)ids.push(checklistsCtrl.checklists[i].id);jQuery.post(getEndpointWithToken(appData.endpoints.order_checklist),{issue_id:appData.issue_id,orders:ids},function(response){1!=response.status&&alert("fail!")})}},$scope.itemsSortableOptions={items:"li:not(.add-new-item-link)",connectWith:"ul.checklist",stop:function(e){var checklistsSorts={};for(var i in checklistsCtrl.checklists){var itemIds=[];for(var j in checklistsCtrl.checklists[i].items)itemIds.push(checklistsCtrl.checklists[i].items[j].id);checklistsSorts[checklistsCtrl.checklists[i].id]=itemIds}jQuery.post(getEndpointWithToken(appData.endpoints.order_items),{issue_id:appData.issue_id,checklists_sort:checklistsSorts},function(response){1!=response.status&&alert("fail!")})}},checklistsCtrl.addNewChecklist=function(){checklistsCtrl.newChecklist.name&&jQuery.post(getEndpointWithToken(appData.endpoints.create_checklist),{issue_id:appData.issue_id,name:checklistsCtrl.newChecklist.name},function(response){return 1!=response.status?void alert("fail!"):(checklistsCtrl.checklists.push({id:response.checklist_id,name:checklistsCtrl.newChecklist.name,editName:checklistsCtrl.newChecklist.name,items:[],completedPercents:0}),checklist.completedPercents,console.log('Created new checklist "'+checklistsCtrl.newChecklist.name+'" with ID: '+response.checklist_id),checklistsCtrl.newChecklist.name="",checklistsCtrl.newChecklist.editMode=!1,void $scope.$apply())})},checklistsCtrl.updateChecklist=function(checklist){return checklist.name==checklist.editName?void(checklist.editMode=!checklist.editMode):(checklist.name=checklist.editName,void jQuery.post(getEndpointWithToken(appData.endpoints.update_checklist),{issue_id:appData.issue_id,id:checklist.id,name:checklist.name},function(response){return 1!=response.status?void alert("fail!"):(console.log("New checklist name: "+checklist.name),checklist.editMode=!checklist.editMode,void $scope.$apply())}))},checklistsCtrl.removeChecklist=function(checklist){for(var i in checklistsCtrl.checklists)checklistsCtrl.checklists[i].id==checklist.id&&(jQuery.post(getEndpointWithToken(appData.endpoints.remove_checklist),{issue_id:appData.issue_id,id:checklist.id},function(response){1!=response.status&&alert("fail!"),console.log('Removed checklist "'+checklist.name+'" with ID: '+checklist.id)}),checklistsCtrl.checklists.splice(i,1))},checklistsCtrl.addNewItem=function(checklist){checklist.newItemText&&jQuery.post(getEndpointWithToken(appData.endpoints.create_item),{issue_id:appData.issue_id,checklist_id:checklist.id,item_text:checklist.newItemText},function(response){return 1!=response.status?void alert("fail!"):(checklist.items.push({id:response.item_id,checked:!1,text:checklist.newItemText,editText:checklist.newItemText}),checklist.completedPercents=getCompletedPercents(checklist.items),console.log('Created new item "'+checklist.newItemText+'" with ID: '+response.item_id),checklist.newItemText="",void $scope.$apply())})},checklistsCtrl.updateItem=function(checklist,item){return item.editText==item.text?void(item.editMode=!item.editMode):(item.text=item.editText,void jQuery.post(getEndpointWithToken(appData.endpoints.update_item),{issue_id:appData.issue_id,checklist_id:checklist.id,item_id:item.id,item_text:item.text},function(response){return 1!=response.status?void alert("fail!"):(console.log('Updated item "'+item.text+'" with ID: '+item.id),item.editMode=!item.editMode,void $scope.$apply())}))},checklistsCtrl.removeItem=function(checklist,item){for(var i in checklist.items)checklist.items[i].id==item.id&&(jQuery.post(getEndpointWithToken(appData.endpoints.remove_item),{issue_id:appData.issue_id,checklist_id:checklist.id,item_id:item.id},function(response){1!=response.status&&alert("fail!"),console.log('Removed item "'+item.text+'" with ID: '+item.id+' in checklist "'+checklist.name+'" with ID: '+checklist.id)}),checklist.items.splice(i,1),checklist.completedPercents=getCompletedPercents(checklist.items))},checklistsCtrl.completeItem=function(checklist,item){item.checked=!item.checked,checklist.completedPercents=getCompletedPercents(checklist.items),jQuery.post(getEndpointWithToken(appData.endpoints.complete_item),{issue_id:appData.issue_id,checklist_id:checklist.id,item_id:item.id},function(response){1!=response.status&&alert("fail!"),response.checked?console.log('Completed item "'+item.text+'" with ID: '+item.id):console.log('Uncompleted item "'+item.text+'" with ID: '+item.id)})},checklistsCtrl.toggleNewChecklistForm=function(){checklistsCtrl.newChecklist.editMode=!checklistsCtrl.newChecklist.editMode},checklistsCtrl.toggleChecklistEdit=function(checklist){checklist.editMode=!checklist.editMode,checklist.editMode&&!checklist.editName&&(checklist.editName=checklist.name)},checklistsCtrl.toggleNeItemEditMode=function(checklist){checklist.newItemEditMode=!checklist.newItemEditMode},checklistsCtrl.toggleItemEditMode=function(item){item.editMode=!item.editMode,item.editMode&&!item.editText&&(item.editText=item.text)};var getCompletedPercents=function(items){var checked=0;for(var i in items)items[i].checked&&checked++;return Math.round(100*checked/items.length)+"%"};for(var i in checklistsCtrl.checklists)checklistsCtrl.checklists[i].completedPercents=getCompletedPercents(checklistsCtrl.checklists[i].items)}]);angularApplication.directive("focusMe",function($timeout,$parse){return{link:function(scope,element,attrs){var model=$parse(attrs.focusMe);scope.$watch(model,function(value){value===!0&&$timeout(function(){element[0].focus()})})}}}),angularApplication.directive("onPressEnter",function(){return function(scope,element,attrs){element.bind("keydown keypress",function(event){13===event.which&&(scope.$apply(function(){scope.$eval(attrs.onPressEnter)}),event.preventDefault())})}}),angularApplication.directive("onPressEsc",function(){return function(scope,element,attrs){element.bind("keydown keypress",function(event){27===event.which&&(scope.$apply(function(){scope.$eval(attrs.onPressEsc)}),event.preventDefault())})}});